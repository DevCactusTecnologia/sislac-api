...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::create('atendimentos', function (Blueprint $table): void {
            ...
            $table->index('status_pagamento', 'idx_atendimentos_status_pg');
            $table->index('unidade_id', 'idx_atendimentos_unidade');
            $table->index(['data', 'id'], 'idx_atendimentos_cursor');
        });

        DB::statement("ALTER TABLE atendimentos ADD CONSTRAINT atendimentos_origem_check CHECK (origem_atendimento IN ('INTERNO', 'WEB_AUTO', 'WEB_APROVADO', 'AGENDAMENTO'))");
        ...
        DB::statement("ALTER TABLE atendimentos ADD CONSTRAINT atendimentos_risco_cardiovascular_check CHECK (risco_cardiovascular IS NULL OR risco_cardiovascular IN ('baixo', 'intermediario', 'alto', 'muito_alto'))");
        ...
        Schema::create('atendimento_exames', function (Blueprint $table): void {
            ...
            $table->index(['atendimento_id', 'status'], 'idx_atendimento_exames_at_status');
            $table->index('exame_id', 'idx_atendimento_exames_exame_id');
        });
        ...
        DB::statement("ALTER TABLE atendimento_exames ADD CONSTRAINT atendimento_exames_cobranca_destino_check CHECK (cobranca_destino IN ('paciente', 'convenio'))");
        ...
        Schema::create('atendimento_pagamentos', function (Blueprint $table): void {
            ...
            $table->index('data', 'idx_atendimento_pagamentos_data');
        });

        Schema::create('atendimento_audit', function (Blueprint $table): void {
            $table->bigIncrements('id');
            $table->text('entidade');
            $table->text('operacao');
            $table->text('acao')->default('');
            $table->unsignedBigInteger('atendimento_id')->nullable();
            $table->unsignedBigInteger('registro_id')->nullable();
            $table->text('protocolo')->default('');
            $table->text('paciente_nome')->default('');
            $table->text('exame_nome')->default('');
            $table->jsonb('old_value')->nullable();
            $table->jsonb('new_value')->nullable();
            $table->uuid('changed_by')->nullable();
            $table->text('changed_by_email')->default('');
            $table->timestampTz('changed_at')->useCurrent();
            $table->text('justificativa')->default('');
            $table->boolean('pos_finalizacao')->default(false);
            $table->boolean('resultado_critico')->default(false);

            $table->index('atendimento_id', 'idx_atendimento_audit_atendimento');
            $table->index('changed_at', 'idx_atendimento_audit_changed_at');
        });
    }

    public function down(): void
    {
        Schema::dropIfExists('atendimento_audit');
