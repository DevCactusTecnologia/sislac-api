...
beforeEach(function () {
    ...
    $this->atendimentoCreateUserId = configureSupabaseTestUser($this, ['criar_atendimento']);
});
...
it('mantém concordância com o baseline quando paciente não possui cpf', function () {
    ...
    $response = $this->postJson('/api/atendimentos', $payload)->assertCreated();
    $id = (int) $response->json('atendimento_id');

    expect((string) supabaseTestPdo()->query("SELECT paciente_cpf FROM atendimentos WHERE id = {$id}")?->fetchColumn())
        ->toBe('');
});
...
it('faz rollback integral quando um filho viola uma invariante do banco', function () {
    ...
    expect(DB::table('atendimentos')->count())->toBe(0)
        ->and(DB::table('atendimento_exames')->count())->toBe(0)
        ->and(DB::table('atendimento_pagamentos')->count())->toBe(0);
});

it('não permite que usuário sem criar_atendimento use o endpoint', function () {
    setSupabaseTestPermissions($this->atendimentoCreateUserId, []);

    $this->postJson('/api/atendimentos', validAtendimentoCreatePayload())
        ->assertForbidden();
});

it('respeita revogação explícita de criar_atendimento', function () {
    denySupabasePermission($this->atendimentoCreateUserId, 'criar_atendimento');

    $this->postJson('/api/atendimentos', validAtendimentoCreatePayload())
        ->assertForbidden();
});

it('rejeita payload estruturalmente inválido antes de persistir', function () {
