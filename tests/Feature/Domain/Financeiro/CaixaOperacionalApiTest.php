...
beforeEach(function () {
    ...
    $this->caixaLaboratory = Laboratory::query()->findOrFail($laboratoryId);
    connectTestLaboratory($this->caixaLaboratory);
    ...
    $this->actingAs($this->caixaUser->fresh(), 'web');
});
...
it('exige gestão financeira para abrir caixa', function () {
    DB::connection('central')->table('users')
        ->where('id', $this->caixaUser->getKey())
        ->where('laboratory_id', $this->caixaLaboratory->getKey())
