...
beforeEach(function () {
    ...
    $this->saidasEstornoLaboratory = Laboratory::query()->findOrFail($laboratoryId);
    connectTestLaboratory($this->saidasEstornoLaboratory);
    ...
    $this->actingAs($this->saidasEstornoUser->fresh(), 'web');
});
...
it('exige gestão financeira para estornar saída', function () {
    ...
    DB::connection('central')->table('users')
        ->where('id', $this->saidasEstornoUser->getKey())
        ->where('laboratory_id', $this->saidasEstornoLaboratory->getKey())
        ->update([
            'role' => 'recepcionista',
